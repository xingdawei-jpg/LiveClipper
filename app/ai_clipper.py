...
★叙事编排核心原则★:
- 同话题片段必须选时间相邻的（SRT条目编号接近），这样上下句才连贯
- 如果同一卖点在不同时间段讲了多次，优先选连续讲的那段（如第15-20条都在讲面料，比分散在第15、80、150条的更连贯）
- 片段A的最后一句和片段B的第一句应该能自然衔接，如果跳到完全不同的话题会觉得断裂
- 宁可牺牲一点"最精彩"，也要保证整条视频像一段完整的话
...
2. Product(产品种草): 选择3-5个片段，全链路消除用户购买顾虑
   ★卖点多样性（最重要）★: 单视频必须覆盖2-4个不同卖点角度（如版型显瘦+颜色显白+穿搭场景+面料），同一角度最多2段，面料最多2段，禁止全片只讲面料
...
[四,片段时长规范]
1. ★★★核心原则：每个片段必须是完整的语义单元，时长由内容自然决定，不要为了凑时长砍断句子★★★
2. 整条视频总时长控制45-65秒——只要总时长在这个范围内，每段多长都行
3. 每段最短2秒（太短一定不完整），最长15秒（太长说明内容太多该拆分）
4. hook: 多长都行，只要爆点完整有力。1-3秒的短爆点最好，4-8秒的完整Hook也行
5. product: 讲完一个完整卖点为止，可能3秒（一句话讲完），可能12秒（举例+对比+效果），都OK
6. close: 促单内容讲完整即可，不要截断尺码引导的中间
   ★close选到行动号召(拍/抢/赶紧/闭眼入)就停，后面的"没有了""断了""好快""我觉得"等收尾废话不要选入★
7. ★不要为了凑某个时长范围而砍句子★——宁可一段8秒讲完一个完整卖点，也不要4秒砍成半句+4秒砍成半句
8. ★片段边界必须对齐句子边界★——不要从句子中间开始，也不要在句子中间结束
9. 总共选8-12个片段
...
7. 结尾片段必须自然收束——最后一句没说完就去掉或往前找完整结束点
8. ★★★叙事连贯性(最重要)★★★:
   - 同话题必须相邻且时间连续:讲面料的放一起、讲版型的放一起、讲颜色的放一起
   - ★优先选SRT条目编号接近的片段★——编号相邻=原文中上下连贯，拼接后语句通顺
   - 如果要讲面料，优先从一段连续讲面料的SRT区域中选取，而不是从3个不同时间点各挑1句
   - 片段A讲完面料，片段B突然跳到尺码=语境断裂；片段A讲面料手感，片段B接着讲面料垂感=连贯递进
   - 编排顺序: 同话题聚拢→话题间用1句过渡→形成"面料手感→面料垂感→版型显瘦→尺码推荐"的递进链
9. 信息密度要高:每3-5秒必须有新信息，禁止超过8秒连续讲同一个点
10. 节奏递进:开头紧凑抓人→中段展开卖点(信息量最大)→后段促单收尾
11. 负面信息后置:"没货了"之类放到结尾区域
...
- 3个方案的Product片段时间范围不重复(同一个时间段只能出现在1个方案中)
- Hook/Close特别精彩的可跨方案共享(最多共享1个)
...
def _call_ai(api_key, base_url, model, srt_text, log_fn, focus_hint=None, srt_entries=None, hook_candidates_hint=None, multi_version=False, return_raw=False, num_versions=3):
    ...
    if _skip_focus:
        ...
        _total_rule = "总时长55-80秒（宁可多选，后续会按版本分配）"
        _hook_rule = "★多版本选片：必须找出3-5个不同类型的Hook候选★ 不要只选1个最强Hook，而是找出圈人群型、极端表态型、痛点型、爆料型、夸奖型各1个（有则选）。不同版本需要不同的Hook开场。"
        _product_rule = "★多版本选片：选择8-12个Product片段★ 覆盖不同卖点角度（版型/面料/功能/风格/品质/对比），每个角度至少1个片段，同一角度有不同表达也要选。"
        _close_rule = "★多版本选片：选择2-3个Close片段★ 不同促单方式（紧迫感/闭眼入/尺码引导/信任强化）各选1个。"
    else:
        _clip_range = "10-15"
...
要求:
1. {_dedup_rule}
2. 每个方案像讲故事一样编排，片段自然衔接，听起来是一段流畅的口播
3. 每个方案8-12个片段，总时长50-70秒
4. ★每个片段严格只选1个编号条目★ 只有当一个条目太短(<3秒)时才可选2个，绝对禁止选3个以上！宁可多选片段也不要选长片段
5. 如果一个片段选的条目之间有间隔，说明中间条目是废话需要跳过——这是正确的
6. {_hook_rule}
7. {_product_rule}
8. {_close_rule}

★Hook选择规则（最重要）★:
- Hook理想长度1-3秒！短爆点比长铺垫有效10倍
- ★扫描SRT中1-3秒的短条目，找含强烈情绪词的作为Hook★
- 最佳范例："太漂亮了！"/"太显瘦了！"/"绝了！"/"这也太惊艳了吧！"/"假不了！"
- 七类Hook(每个方案用不同类型):
  ① 痛点提问: "胯宽腿粗还在乱穿？"、"腰腹有肉穿啥显壮？" → 最强
  ② 效果前置: "原相机上身太显瘦了"、"130斤穿这条秒变沙漏腰" → 视觉最强
  ③ 对比反差: "同样是裙子这款收腰遮肉差别太大" → 冲击力最强
  ④ 悬念福利: "别划走！这件我只在直播间穿一次" → 拉停留
  ⑤ 爆料型: "XX%是假的"、"行业秘密" → 强停留
  ⑥ 信任型: "被扣爆了"、"卖疯了"、"盲拍" → 拉信任
  ⑦ 夸奖型: "太好看"、"绝绝子" → 最弱但可用
  ⑧ 包容承诺型: "我不管你"、"我都给你"、"想怎么穿都行"、"随便你" → 重复强调+包容性语气
- ★各方案必须用不同类型的Hook★
...
{{
  "versions": [
    {{
      "angle": "方案角度描述",
      "clips": [
        {{"clip_type": "hook", "srt_indices": [3, 4], "focus": "痛点提问", "reason": "选择理由"}},
        {{"clip_type": "product", "srt_indices": [8, 9], "focus": "版型显瘦", "reason": "选择理由"}},
        ...
      ]
    }},
    ...
...
4. ★每个片段严格只选1个编号条目★ 只有当一个条目太短(<3秒)时才可选2个，绝对禁止选3个以上！每个片段5-10秒最佳
...
★输出格式★: 每个片段用 srt_indices 字段指定选了哪些编号条目（数组），不要填start/end时间戳。★每个片段只选1-2个条目，确保单片段5-10秒，不要选3个以上★:
[
  {{"clip_type": "hook", "srt_indices": [3, 4], "focus": "痛点提问", "reason": "圈人群+效果对比"}},
  {{"clip_type": "product", "srt_indices": [8, 9, 10], "focus": "版型显瘦", "reason": "上身效果最强"}},
  ...
]
...
{indexed_transcript}"""
    ...
    try:
        ...
        if return_raw:
            return _parse_raw_response(content, log_fn)
        return _parse_ai_response(content, log_fn, srt_entries)
...
def _parse_ai_response(content, log_fn, srt_entries=None):
    def _log(msg):
        if log_fn: log_fn(msg)
    ...
    for idx, item in enumerate(data):
        ...
        if srt_idx and _srt_entry_map:
            ...
            if isinstance(srt_idx, int):
                srt_idx = [srt_idx]
            # 排序+拆分不连续索引为多个clip
            valid_indices = sorted(set(i for i in srt_idx if 1 <= i <= len(_srt_entry_map)))
...
# 多版本格式解析
# ============================================================
def _parse_multi_version_data(data, log_fn, srt_entries=None):
...
def ai_analyze_multi_versions(srt_text, log_fn=None, force_category=None, focus_hint=None, num_versions=3):
    ...
    # ★单次AI调用，直接出多版本方案★
    _log(f"AI: 调用 {model} 多版本模式(1次调用出{num_versions}个方案)...")
    raw_data = _call_ai(api_key, base_url, model, cleaned_srt, _log,
    ...
    if not raw_data:
        ...
        # Fallback: use old 3-call approach
    ...
    # ★解析多版本响应★
    all_versions = _parse_multi_version_data(raw_data, _log, _indexed_srt_entries)

    if not all_versions or not all_versions.get("versions"):
        _log("AI: 多版本解析失败，降级到3次单版本调用...")
        return _multi_version_fallback(srt_text, _log, force_category, focus_hint, num_versions,
                                       api_key, base_url, model, cleaned_srt, _indexed_srt_entries,
                                       _hook_hint)
    ...
    for vi, ver in enumerate(all_versions["versions"][:num_versions]):
        ...
        # 语义重复（多版本模式跳过）
        # clips = _filter_semantic_repeat(clips, _log)
        ...
        if not clips or len(clips) < 3:
            _log(f"方案{vi+1}: 后处理后不足3段，跳过")
            continue

        total_dur = sum(c[5] for c in clips)
        _log(f"✅ 方案{vi+1} [{ver.get('angle', '?')}]: {len(clips)}片段, {total_dur:.1f}s")
        processed_versions.append({"angle": ver.get("angle", focus_hint or "综合"), "clips": clips})
...
def _multi_version_fallback(srt_text, log_fn, force_category, focus_hint, num_versions,
    ...
    import random
    all_angle_hints = [
        ("版型显瘦", "以版型显瘦开场（Hook+前1-2个Product讲显瘦/遮肉/修饰身形/遮胯/收腰），后续Product必须覆盖颜色/穿搭/品质等其他卖点，同一角度最多2段，面料最多2段"),
        ("颜色显白", "以颜色显白开场（Hook+前1-2个Product讲颜色/显白/衬肤色/提气色），后续Product必须覆盖版型/显瘦/穿搭等其他卖点，同一角度最多2段，面料最多2段"),
        ("穿着场景", "以穿着场景开场（Hook+前1-2个Product讲通勤/约会/实穿/百搭场景/懒人穿搭），后续Product必须覆盖版型/显瘦/颜色等其他卖点，同一角度最多2段，面料最多2段"),
        ("性价比", "以性价比开场（Hook+前1-2个Product讲品质对比/价值锚点/闭眼入/大牌平替），后续Product必须覆盖版型/显瘦/场景等其他卖点，同一角度最多2段，面料最多2段，禁止报具体价格"),
        ("紧迫稀缺", "以紧迫感开场（Hook+前1-2个Product讲库存/限时/稀缺/手慢无），后续Product必须覆盖版型/颜色/显瘦等其他卖点，同一角度最多2段，面料最多2段"),
        ("情绪感染", "以情绪感染开场（Hook+前1-2个Product选最激动/最真诚/最惊艳的片段，如超级超级/太漂亮了/美爆了），后续Product必须覆盖版型/显瘦/颜色等实际卖点，同一角度最多2段，面料最多2段"),
        ("流行趋势", "以流行趋势开场（Hook+前1-2个Product讲设计感/当季流行/风格定位/法式韩系），后续Product必须覆盖版型/显瘦/实穿等其他卖点，同一角度最多2段，面料最多2段"),
        ("面料质感", "以面料质感开场（Hook+前1-2个Product讲面料/手感/品质/亲肤感），后续Product必须覆盖版型/显瘦/颜色/穿搭等其他卖点，同一角度最多2段，★面料总共最多1段★"),
    ]
    ...
    for vi, (angle_name, angle_hint) in enumerate(angle_hints):
        ...
        if vi > 0:
            if used_hook_types:
                _extra_hint += f"\n★前面版本已用[{', '.join(used_hook_types)}]类型的Hook，你必须选其他类型★"
            if used_close_types:
                _extra_hint += f"\n★前面版本已用[{', '.join(used_close_types)}]类型的Close，你必须选其他类型★"
        ...
        # 记录Hook/Close类型
        _hook_types = {"爆料": ["假的", "秘密", "不敢信"], "痛点": ["显瘦", "遮肉", "藏肉", "不挑"],
                       "信任": ["盲拍", "闭眼入", "不搞虚"], "夸奖": ["绝了", "天花板", "卖疯"],
                       "背书": ["秀场", "大牌", "平替"]}
        _close_types = {"尺码": ["尺码", "卡码"], "信任": ["一定", "真的"], "风格": ["洋气", "高级"],
                        "促单": ["冲", "入手", "抢"], "优惠": ["到手", "破价"]}
        for c in clips:
            ct = c[0]
            txt = c[1] if isinstance(c, (list, tuple)) and len(c) > 1 else ""
            if ct == "hook" and txt:
                for ht, kws in _hook_types.items():
                    if any(kw in txt for kw in kws):
                        if ht not in used_hook_types: used_hook_types.append(ht)
                        break
            elif ct == "close" and txt:
                for ct2, kws in _close_types.items():
                    if any(kw in txt for kw in kws):
                        if ct2 not in used_close_types: used_close_types.append(ct2)
                        break
...
def _dedup_clips(clips, log_fn, multi_version=False, focus_hint=None):
