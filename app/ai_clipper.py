...
4. 即使其他品类的文案再好，也不能选
5. 例如:全场裤子出现最多，则只选裤子的片段，禁止选上衣/裙子的片段

[一,强制数量约束]
1. 必须输出[12-20段]片段，绝对不能少于10段，尽量多选优质片段
2. 总时长严格控制:45-65秒(超时必须删除冗余段落,不要拖沓)
3. 如果主打单品的同类型好片段不够，宁可重复不同角度的卖点，也不要混入其他品类

[二,黄金链路结构]
采用"必选+可选"灵活组合模式，而非固定环节顺序：

【必选环节】（必须覆盖，缺一不可）
1. Hook(开头抓人): ★★★ 最关键环节，决定用户是否划走 ★★★
   - 必须是独立完整的一句话，能在3秒内传达一个强烈信息
   - 五类高价值Hook(优先级从高到低):
     ① 爆料型(最强停留): "XX%是假的"、"行业秘密"、"全都是假的" → 制造好奇
     ② 痛点型(精准锁客): "显瘦"、"显白"、"腿粗也能穿"、"想穿XX" → 直击需求
     ③ 信任型(强信任): "信XX可盲拍"、"不搞虚"、"精挑细选" → 拉信任
     ④ 夸奖型(情绪感染): "被扣爆了"、"卖疯了"、"巨好看" → 激发兴趣
     ⑤ 背书型(品味暗示): "对标秀场款"、"同款"(注意:品牌名必须剔除) → 提升期待
   - ★绝对禁止★:话头接续句("就像我的话...","然后...","所以呢...","看一下...","来...","好..."),不完整半句,平淡开场("这个面料真的很好看"、"来给大家看一下"、"这件衣服很OK")
   - ★Hook最低标准★:如果这句话单独出现在抖音信息流,用户会不会停留?不会就换一条
   - ★必须搜遍全片选最强Hook★:不要偷懒选时间最早的第一句,要在整个字幕中找出冲击力最强的那句作为开头
   - 钩子判断标准:假设只有这一句话，用户会不会停下来看?如果会，就是好钩子
   - ★Hook选片铁律★:
     a) 不要优先选时间最早的第一句!直播开头80%是暖场废话("然后这个..."、"这个位置..."、"它是属于...")
     b) 在整个字幕中搜索五类高价值Hook,选信息密度最高的那个,而非时间最早的
     c) 允许合并相邻2-3句字幕提炼成一句更有冲击力的Hook
     d) Hook测试:把这句话单独念给朋友听,对方会想继续了解吗?如果不会,换一条
     e) 好Hook vs 坏Hook对比:
        好Hook:"肩宽背厚穿起来很薄很瘦"(痛点+效果,2秒说完)
        好Hook:"接完这一单就不接了"(稀缺感+紧迫感)
        好Hook:"薄到不能再薄了"(极端描述,有画面感)
        坏Hook:"这个面料真的很好看"(平淡陈述,没有钩子)
        坏Hook:"然后这一整身"(接续句,信息为零)
        坏Hook:"它是属于非常适合办公室人群去穿的"(太长太淡,没有冲击力)

2. Product(产品种草): 选择2-4个片段，全链路消除用户购买顾虑
   以下类型优先级从高到低，根据原视频实际内容灵活选择(不强求每个类型都有):
   ① 痛点解决(最强转化): "腿粗绝对可以穿进去"、"120斤也能穿" → 直接回答用户顾虑
   ② 对比突出(差异化种草): "比市面上的加宽"、"市面版本拉不开" → 突出优势
   ③ 版型/面料/细节: 讲解设计、材质、做工 → 建立产品认知
   ④ 穿搭展示: 多种风格搭配、跨季节可穿 → 证明百搭实穿性
   ⑤ 价格锚点: "外面卖七八千"、"同款XX块钱" → 营造价值感(非直接报价格)
   每个片段应覆盖用户关心的一个具体问题，避免空洞描述

3. Close(促单收尾): 选择2-3个片段，降低决策成本促成下单
   - ★尺码/价格/购物车引导只能放在Close区域(最后2-3段)★
   - 绝对不能在hook或product区域报尺码、报价格、引导加购物车
   - 尺码推荐: "按推荐尺码买就行"、"有弹力卡码往小拍" → 消除尺码顾虑
   - 价格信息: "折后XX"、"到手XX"、"成套买更划算" → 价格引导
   - 信任强化: "一定是真的"、"精挑细选" → 最终信任确认
   - 风格定位: "韩剧女主"、"洋气"、"高级" → 帮用户建立穿搭期待
   - 如果原视频有尺码/价格引导，必须放在最后

【可选环节】（有就保留，没有不强求，不要硬凑）
- Bridge(过渡衔接): 科普类("教你辨别真假")、提问类("为什么大家都喜欢") → 连接Hook和Product，建立专业感
- 信任话术: "盲拍"、"不搞虚"、"自留款"、"私服" → 可穿插在任意环节，不要独立成段
- trend(流行趋势): 当季流行、设计款、不撞款 → 有则保留，无则跳过

[三,绝对禁止]
1. 禁止只选1-7段，最低8段
2. 禁止选过渡废话:再开新款,过款,接下来,好吧
3. 禁止混入其他品类的片段——这是最容易犯的错误，请反复检查每个片段是否属于主打单品
4. ★禁止选不完整的半句话★:每个片段的首尾必须是完整句子的边界
5. ★禁止选主播回弹幕的废话★:如"没有的啊","没有的事","知道知道"等纯互动内容
6. ★禁止选纯ASR错误片段★:如果一段话读起来完全不通，说明是ASR识别错误，跳过
7. 禁止选语句不完整的片段:一句话说到一半就断了，缺少主语、谓语、补语或逻辑衔接
   - [红线]最后一个片段必须以完整的句子结尾，禁止以"你会觉得""就感觉""呢""吧""啊"等未完成的语气结束
   - 判断标准:这句字幕如果单独展示，观众能理解完整意思吗?不能则排除
   - 例外:如果前后相邻字幕能自然衔接成一个完整句子，可以合并选择
8. ★禁止保留具体品牌名★:如"香奈儿""古驰""爱马仕"等大牌名称必须从文案中剔除，替换为通用描述(如"大牌平替""秀场款")或直接删除
9. ★禁止语义重复★:同一个卖点用不同说法说了多遍，只保留信息量最高的第一次
   - 但注意:连续的夸奖堆叠(如"太好看了""绝了""巨好看")是有效的种草手法，不应被判定为重复
   - 真正的重复:同一事实/卖点用不同话术复述(如"面料不厚"→"比较薄透"→"很轻盈")
   - 例:"推荐给通勤的女生"出现3次、"现货48小时秒飞"出现3次 → 只保留第一次
10. 禁止选无实质内容的语气词:单独的"嗯""对""啊""当然"等无价值感叹词
11. ★禁止同主题内容被无关内容打断★:如果选了3段讲面料的片段,它们必须相邻排列,中间不能插入尺码/价格/其他话题
12. ★禁止选纯过渡废话★:"然后反正这身"、"好的然后"、"来给大家看一下"等没有实际信息的过渡句
13. ★禁止尺码/价格信息出现在前半段★:尺码推荐、价格报数、"加购物车"只能在最后2-3段

[四,输出格式]
只输出 JSON 数组，每个片段必须包含 focus 字段标注主打卖点:
[
  {"clip_type": "hook", "start": "秒数", "end": "秒数", "text": "修正后的文案，用【】标注重点词如【超级无敌冰】", "focus": "版型"},
  ...
text字段要求：用中文【】符号包裹该片段最核心的1-3个关键词（产品卖点、价格、感受词等），字幕会自动将【】内的词标黄放大
]
start/end 使用输入字幕中的真实时间戳.

clip_type 从以下类型中选择(与第二节环节对应):
- "hook": 开头抓人片段
- "product": 产品种草片段(版型/面料/穿搭/对比/痛点解决等)
- "close": 促单收尾片段(尺码/信任/价格引导等)
- "bridge": 过渡衔接片段(科普/提问，可选)

focus 字段(必填)从以下卖点中选择最贴切的一个:
- "版型": 版型、剪裁、廓形、显瘦遮肉等版型相关
- "面料": 材质、手感、触感、起球、克重等面料相关
- "颜色": 颜色、花色、图案、条纹等外观相关
- "显瘦": 显瘦、显高、遮胯、藏肉、修饰身材
- "场景": 通勤、约会、度假、日常等穿着场景
- "搭配": 搭配、组合、套穿、配什么等搭配建议
- "对比": 对比市面产品、反面案例对比、价格锚点
- "品质": 做工、走线、细节、质感等品质相关
- "价格": 价格、到手价、性价比、优惠
- "痛点解决": 直接回答用户顾虑(如"腿粗也能穿")
- "信任": 信任话术(盲拍、不搞虚、自留款、私服)
- "其他": 不属于以上类别的片段
如果片段涉及多个卖点，选最核心的那一个.

[五,叙事连贯性(核心)]
1. 每个片段必须是完整的一句话或一个完整的意思，不能截断到一半
2. 片段之间的衔接要有递进感:上段的内容自然引出下段
   - 推荐结构:hook(抓人)→ product(种草，同主题相邻)→ close(促单)
3. 禁止相邻片段内容重复或高度相似(比如连续两个片段都在说价格)
4. 每段片段的文案应该是"能直接作为短视频配音"的流畅语句
5. 片段时间范围要自然——宁可多包含1-2秒废话，也不要把一句话切断
6. 结尾片段必须自然收束，不能卡在半句话上.如果最后一句没说完，去掉它或者往前找一个完整的结束点
7. ★同主题必须相邻★:所有讲面料的段落放一起、讲版型的放一起、讲颜色的放一起，不要穿插。如果原视频中同一主题的片段分散在不同时段，也要在输出时把它们聚拢到一起
8. ★尺码/价格/促单信息严格后置★:所有尺码推荐、价格信息、"加购物车"必须放在最后2-3段

[六,片段时长优化]
1. hook 片段(必须第一个输出!): 2-4秒(快速抓人), 绝不能放在中间或结尾
2. product 片段 5-10秒(有内容有细节)
3. bridge 片段 3-6秒(简短过渡)
4. close 片段 3-8秒(促单有力)
5. 总时长严格控制 45-65秒，超出65秒必须删除冗余段落

[七,短视频节奏感]
1. 信息密度要高:每3-5秒必须有新信息(新卖点/新对比/新穿搭)，禁止超过8秒连续讲同一个点
2. 节奏递进:开头紧凑抓人 → 中段展开卖点(信息量最大)→ 后段促单收尾
3. ★紧凑原则★:每段片段必须包含实质性信息(卖点/痛点/效果/数据),纯过渡废话一律不选
   - 不要选:"好的然后"、"来给大家看一下"、"然后反正这身"、"大家可以看"
   - 要选:有具体卖点的完整句子
4. 尊重原始叙事流:优先按原视频逻辑选片编排，但同主题片段要聚拢
5. 负面信息后置:如果不得已要选"没货了"这类内容，放到结尾区域，不要放在开头

[八,ASR纠错]
你输出的 text 字段必须是修正后的流畅文案:
1. 根据上下文修正明显的ASR识别错误(如"惊恐"应为"惊艳"等)
2. 去掉不必要的语气词和重复(如连续两个"绝了"合并为一个)
3. 保留口语化风格，但确保读起来通顺自然
4. 如果不确定原文意思，宁可不修正也不要改错
5. 重复检测:检查选出的所有片段，如果两条片段的核心信息相同(只是换了个说法)，只保留更完整的那条

[九,质量自检(输出前必须执行)]
1. Hook检查:第一段是否能让陌生人停下来?如果太淡(如"然后这一整身"),在整个字幕中重新找一条最有冲击力的
2. 重复检查:是否有相同信息出现2次以上?(如"推荐通勤女生"出现多次、"现货秒飞"出现多次)只保留第一次
3. 时长检查:总时长是否超过65秒?如果是,优先删除:重复段→过渡废话段→信息量最低的product段
4. 内容检查:每段是否有实质内容?纯语气词/纯过渡句(如"好的然后""来给大家看")删除
5. 位置检查:尺码/价格/购物车引导是否都在最后2-3段?如果在前半段,移到末尾
6. 连贯性检查:同主题内容是否相邻?如果面料段被其他话题隔开,重新排列使其聚拢
7. 结尾检查:最后1-2段是否包含促单信号?如没有,找一个合适的促单内容作为结尾
8. 完整性检查:每段是否是完整句子?半截话(缺主语/宾语/转折词后无内容)删掉
9. ★品牌名检查★:文案中是否有具体品牌名(如"香奈儿""古驰"等)?替换为通用描述或删除
"""
...
要求:
1. ★绝对禁止重复同一卖点★ 字幕中主播会重复讲同一个卖点(如"面料好"说了3遍)，你必须只选每个卖点的最佳版本，严禁选两段内容相似的片段
2. 像讲故事一样编排，每个片段自然衔接下一段，听起来是一段流畅的口播
2. 精选10-15个片段，宁可多选也不要少选，总时长40-65秒（短一点没关系，内容丰富更重要）
...
def _dedup_clips(clips, log_fn):
    ...
    other_clips_price = [c for c in clips if c not in price_clips]
    if price_clips:
        clips = other_clips_price + price_clips
        _log(f"价格后置: {len(price_clips)} 个价格片段移到末尾")
        # 保护前3个位置不被价格内容占据
        for i in range(min(3, len(clips))):
            if any(kw in clips[i][1] for kw in price_keywords):
                for j in range(i + 1, len(clips)):
                    if not any(kw in clips[j][1] for kw in price_keywords):
                        clips[i], clips[j] = clips[j], clips[i]
                        _log(f"价格保护: 位置{i+1}的价格片段与位置{j+1}交换")
                        break

    # close类型保护: 前3个位置不允许close类型片段(兜住AI标错的情况)
    for i in range(min(3, len(clips))):
        if clips[i][0] == "close":
            for j in range(i + 1, len(clips)):
                if clips[j][0] != "close":
...
def _fix_clip_boundaries(clips, cleaned_srt, log_fn=None):
    ...
    if fix_count:
        ...
        if len(fixed_clips) >= 2:
            fixed_clips.sort(key=lambda c: c[2])  # 按start排序
            overlap_fixed = 0
