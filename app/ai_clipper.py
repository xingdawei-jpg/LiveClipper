...
1. Hook(开头抓人): ★★★ 最关键环节，决定用户是否划走 ★★★
   ...
   - ★必须搜遍全片找所有Hook候选★:不要只找1个最强的，要找出所有可作为Hook开场的句子(3-5个)，不同类型(圈人群/极端表态/痛点/爆料/夸奖)各选最佳1个
   - ★Hook选片铁律★:
...
2. Product(产品种草): 选择3-5个片段，全链路消除用户购买顾虑
   ...
   优先级从高到低(不强求每个类型都有):
   ① 价值锚点(最强种草，爆款必选): "这个品质外面买不到"、"同款对比贵一倍"、"闭眼入不踩雷" → 强调"值"的印象，绝对不报具体价格
...
3. Close(促单收尾): 选择1-2个片段，核心是引导下单
   ...
   - 🚫价格/购物车/链接/下单/优惠/限时价内容绝对禁止选择——无论Hook/Product/Close任何位置都禁止★
   - 如果原视频有尺码引导，必须放在最后
...
3. 精选{_clip_range}个片段，宁可多选也不要少选，{_total_rule}
4. 每个片段包含完整的语句，不要切断正在说的话
5. [时间范围] 字幕时间范围 {_srt_min:.0f}s - {_srt_max:.0f}s，你的 start/end 必须严格在此范围内，禁止编造超出此范围的时间戳
6. [本轮选片偏好]{focus}
7. {_hook_rule}
8. {_product_rule}
9. {_close_rule}
