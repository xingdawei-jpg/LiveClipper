...
4. ★★★每个片段严格只选1个编号条目★★★ 选2个条目=片段太长=废话多=成品差！宁可9段各1个条目(每段3-8秒)，也不要5段各2-3个条目(每段12-15秒)。短而精>长而散
...
9. ★Product片段允许跨方案共享(最多共享2段)，但Hook和Close不可共享★ 这样每个版本都有足够的种草素材，不会因为素材隔离导致内容单薄
10. ★每个方案必须独立完整: 1 Hook + 2-4 Product + 1 Close★
...
def ai_analyze_multi_versions(srt_text, log_fn=None, force_category=None, focus_hint=None, num_versions=3):
    ...
    # Build forbidden indices (price/CTA/forbidden words)
    _forbidden_indices = set()
    if _indexed_srt_entries:
        _fw_data_mv2 = load_keywords()
        _fw_words_mv2 = _fw_data_mv2.get('forbidden_phrases', [])
        _price_patterns_mv2 = [
            re.compile(r'\d{2,4}\s*[\u5143\u5757]'),
            re.compile(r'[\u53ea\u5c31]\u8981[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e\u5343\u4e07\u8d2f\d]+[\u5143\u5757]'),
            re.compile(r'[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e\u5343\u4e07\u8d2f\d]+[\u591a\u6765\u5757]?[\u5143\u5757]'),
            re.compile(r'\u539f\u4ef7|\u62a2\u5230|\u51b2\u5230|\u63a8\u8350|\u9650\u65f6\u4ef7'),
            re.compile(r'\u4e70\u5230|\u8d5a\u5230|\u53bb\u62a2|\u8d76\u7d27|\u5237\u5230'),
        ]
        for _fi, (_fse, _fee, _fte) in enumerate(_indexed_srt_entries, 1):
            _mfw = [w for w in _fw_words_mv2 if w in _fte] if _fw_words_mv2 else []
            _mp = any(p.search(_fte) for p in _price_patterns_mv2) if _price_patterns_mv2 else False
            if _mfw or _mp:
                _forbidden_indices.add(_fi)

    # ★解析多版本响应★
    all_versions = _parse_multi_version_data(raw_data, _log, _indexed_srt_entries, _forbidden_indices)
