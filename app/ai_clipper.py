...
2. 总时长严格控制:45-60秒(不超过1分钟,超时必须删除冗余段落,不要拖沓)
...
1. Hook(开头抓人): ★★★ 最关键环节，决定用户是否划走 ★★★
   ...
   - 五类高价值Hook(优先级从高到低):
     ① 圈人群型(最强Hook): "屁股大的看过来"、"腿粗的看过来"、"110斤显瘦80斤"、"肩宽背厚的"、"小个子也能穿"、"胯宽的姐妹" → 精准锁定目标人群,这是转化最高的Hook,比用产品/面料开场精准10倍
     ② 极端表态型(强好奇): "吃土都要买"、"经典中的经典"、"必入"、"闭眼入"、"不买后悔" → 极端判断制造强烈好奇
     ③ 痛点型(精准锁客): "显瘦"、"显白"、"腿粗也能穿"、"想穿XX" → 直击需求
     ④ 爆料型(强停留): "XX%是假的"、"行业秘密"、"全都是假的" → 制造好奇
     ⑤ 夸奖+信任型(情绪感染): "被扣爆了"、"卖疯了"、"盲拍"、"不搞虚" → 激发兴趣+拉信任
   - ★绝对禁止★:用产品/面料/款式做Hook开场(如"这个西装"、"这件风衣"、"面料很好")——人群圈定比产品介绍重要,泛流量不如精准流量
   ...
   - ★Hook选片铁律★:
     ...
     e) 好Hook vs 坏Hook对比(来自真实爆款切片):
        好Hook:"屁股大的看过来"(圈人群,精准锁客)
        好Hook:"110斤显瘦80斤"(圈人群+效果对比,信息量爆表)
        好Hook:"腿粗的也能穿进去"(圈人群+痛点解决)
        好Hook:"吃土都要买一条"(极端表态,强好奇)
        坏Hook:"这个西装真的很好看"(产品开场,人群太泛)
        坏Hook:"面料很舒服"(面料开场,没有圈人群)
...
2. Product(产品种草): 选择3-5个片段，全链路消除用户购买顾虑
   ...
   ① 价值锚点(最强种草,几乎是爆款必选): "原版16000→我们300来块"、"外面卖七八千"、"同款XX块钱"、"省下5万"、"XX代工厂" → 先用高价锚定,再给我们的价格,差距越大冲击力越强。9个爆款案例中8个有此元素
   ② 代工厂/产地背书(强信任): "一线品牌代工厂"、"给XX做婚纱的工厂"、"意大利工艺"、"澳洲美丽奴" → 不提品牌名但信任感拉满,5/9爆款有此元素
   ③ 痛点解决(强转化): "腿粗绝对可以穿进去"、"120斤也能穿" → 直接回答用户顾虑
   ④ 对比突出(差异化种草): "比市面上的加宽"、"市面版本拉不开" → 突出优势
   ⑤ 版型/面料/细节: 讲解设计、材质、做工 → 建立产品认知
   ⑥ 场景想象(画面感): "法国女生的浪漫感"、"办公室喝茶的雅"、"婴儿肌肤一样" → 说穿上后的感觉,不说参数,比单纯描述面料更打动人
   ⑦ 穿搭展示: 多种风格搭配、跨季节可穿 → 证明百搭实穿性
   每个片段应覆盖用户关心的一个具体问题，避免空洞描述

3. Close(促单收尾): 选择1-2个片段，核心是引导下单
   - ★尺码引导只能放在Close区域(最后1-2段)★
   - 绝对不能在hook或product区域报尺码
   - 促单(最有效Close): "闭眼冲"、"剩下不多"、"300件拍完就没有了"、"不买真没了" → 用紧迫感+高级语言引导下单
   - 闭眼入(强信心): "闭眼入"、"不买后悔" → 用极致断言代替犹豫
   - 尺码推荐: "按推荐尺码买就行"、"卡码买小不要买大" → 消除尺码顾虑
   - 信任强化: "一定是真的"、"没人舍得退" → 最终信任确认
   - ★价格/购物车/链接内容绝对禁止选择★:成品不能出现任何价格/购物车/下单/链接相关内容
   - 如果原视频有尺码引导，必须放在最后
...
13. ★禁止尺码/价格信息出现在前半段★:尺码推荐只能在最后2-3段;价格/购物车/下单内容绝对禁止选择
...
8. ★尺码严格后置★:所有尺码推荐必须放在最后2-3段;价格/购物车/下单内容绝对不能出现
