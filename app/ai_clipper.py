...
1. 品类统计→确定主打单品 2. Hook扫描→选冲击力最强的 3. 链路规划→hook/product/close分配 4. 重复排查→去重叠 5. 直接输出JSON
...
3. Close(促单收尾): ★必须选择1-2个片段★核心是消除顾虑+推动决策，绝不能空缺
   ...
   - 🚫绝对禁止:具体价格数字(199/299等)、购物车、链接、下单、优惠、限时价、321上车
...
def ai_analyze_multi_versions(srt_text, log_fn=None, force_category=None, focus_hint=None, num_versions=3):
    ...
    for vi, ver in enumerate(all_versions["versions"][:num_versions]):
        ...
        clips = _ensure_close_complete(clips, cleaned_srt, _log)
