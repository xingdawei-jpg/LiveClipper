...
2. Product(产品种草): 选择6-10个片段，全链路消除用户购买顾虑
   ...
   ⚠️面料描述避嫌(重要): 成品视频中禁止出现具体面料成分名称(真丝/桑蚕丝/棉/聚酯纤维/涤纶/莫代尔/天丝/棉麻/亚麻/醋酸/铜氨/腈纶/锦纶/氨纶/粘纤/人造棉/人棉等)。可以用泛化描述("质感好""面料高级""垂感好""凉感""冰感""手感好""软糯""有筋骨"),也可以用品牌方专属命名(如"醋酸冰感""牛奶丝")。如果选到的片段全是讲具体面料的，宁可换角度(版型/颜色/场景)也不要播报具体面料成分。核心:用户知道"面料好"就够了，不需要知道"是什么面料"。
...
def ai_analyze_clips(srt_text, log_fn=None, force_category=None, multi_version=False, focus_hint=None, hook_candidates_hint=None):
    ...
    for attempt in range(3):
        ...
        # clips = _extract_hook_from_products(clips, cleaned_srt, log_fn)  # [David] AI自己选Hook，代码不插手
...
def ai_analyze_multi_versions(srt_text, log_fn=None, force_category=None, focus_hint=None, num_versions=3):
    ...
    _angle_hints_map = {
        "版型显瘦": "Hook: 选目标人群爆点句(微胖/遮肉/显瘦优先，1-3秒定位人群)。Product叙事: 痛点定位→解决方案→面料佐证→版型优势→尺码收尾。先明确为谁解决什么问题(胖MM/梨形/微胖要遮肉)，再用面料增信任，最后尺码建议",
        "颜色氛围": "Hook: 选颜色显白/黄皮友好型圈人群爆点句。Product叙事: 颜色效果→穿着体感→适用场景→搭配方案。先讲颜色显白提气色，再穿感舒适，再接场景搭配建议",
        "穿着场景": "Hook: 选场景化爆点句(通勤/约会/见家长优先)。Product叙事: 场景痛点→选品理由→功能细节→搭配参考。先说明为什么这个场景需要这件衣服",
        "性价比": "Hook: 选品质对比类爆点，禁止出现具体价格。Product叙事: 品质认知→标杆对比→不可替代性→推荐理由。先建立品质感再对标商场款强调物超所值",
        "情绪感染": "Hook: 选极致情绪爆点(绝了/太美了/封神优先)。Product叙事: 情绪引爆→亮点拆解→实物证明→情绪收尾。情绪类无需太多干货，保持节奏感",
        "流行趋势": "Hook: 选风格标签类爆点(法式/韩系/新中式)。Product叙事: 风格定调→核心单品→完整搭配→适配人群。先定风格调性再选单品细节再给完整搭配方案",
        "面料质感": "Hook: 选触感圈人群爆点(像云朵/婴儿肌/软糯优先)。Product叙事: 材质来源→触感体验→透气耐穿→品质保证→推荐理由。先讲真材实料建立信任再促成",
    }
    ...
    for vi, _angle in enumerate(_version_angles):
        ...
        # [David] AI自己选Hook
        # # _extract_hook_from_products(clips, cleaned_srt, _log)
...
def _relax_clips(clips, log_fn):
    ...
    for c in clips:
        if c[0] not in type_best or c[4] > type_best[c[0]][4]:
            type_best[c[0]] = c
    # Use AI's original clip_order instead of GOLDEN_CHAIN type-sorting
    # to preserve narrative coherence (GOLDEN_CHAIN breaks sentence continuity)
    sorted_clips = clips[:] if len(clips) >= 2 else [type_best[t] for t in GOLDEN_CHAIN if t in type_best]
