...
class BuildV4FullPackageTests(unittest.TestCase):
    ...
    def test_build_rejects_a_target_that_differs_from_frozen_identities(self) -> None:
        self.assertEqual(package_builder.DEFAULT_CORE_VERSION, "4.0.0")
        package_builder.validate_embedded_core_version("4.0.0")
        with patch.object(package_builder, "HOST_CORE_VERSION", "4.0.1"):
            with self.assertRaisesRegex(RuntimeError, "embedded Host identity"):
                package_builder.validate_embedded_core_version("4.0.0")
        with patch.object(package_builder, "LAUNCHER_VERSION", "4.0.1"):
